...
def tma_tile_g2s(
    ...
    *,
    ...
    """Standard 2D TMA tile copy from global to shared memory.

    Loads one contiguous tile of boxDim=(width, height) at the specified
    (col, row) from the tensor described by tmap_ptr. Used for loading
    contiguous pages from paged KV cache.
    """
    llvm.inline_asm(
        None,
        [
